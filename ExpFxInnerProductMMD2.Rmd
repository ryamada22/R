...
Maximum Mean Discrepancy (MMD)はカーネル法により忠実な分布間異同定量measureである。
以下のように、$IP$を内積としてみたときの、「距離の２乗」そのものとなっている。

$$
MMD(P_1(x|\theta_1),P_2(x|\theta_2)) = IP(P_1(x|\theta_1),P_1(x|\theta_1)) + IP(P_2(x|\theta_2),P_2(x|\theta_2)) - 2 IP(P_1(x|\theta_1),P_2(x|\theta_2))
$$
この$MMD$が指数型分布族にて、うまい挙動をするような$\psi$関数について考えてみる。

以下のように$MMD$を$IP(P_1(x|\theta_1),P_2(x|\theta_2)) = \frac{e^{\psi(\theta_{1+2})}}{e^{\psi(\theta_1)}e^{\psi(\theta_2)}}$を用いて、あらわすことにする。
...
$$
IP(P_1(x|\theta_1),P_2(x|\theta_2)) = \frac{f(\theta_1+\theta_2)}{f(\theta_1)f(\theta_2)}
$$

天下り式となるが、f(\theta) = \sin^2{\theta}
# Rでやってみる(MDSの手前まで)
